...
__run() {
    ...
    sh -c 'bash -c "$(curl -sS https://raw.githubusercontent.com/EasongChung/cdn/main/mht-arm64-install.sh)"'

    # sh -c 'if [ -f "/kuaicdn/app/ipes/bin/ipes" ]; then /kuaicdn/app/ipes/bin/ipes start; fi;tail -f /dev/null'
    # bash -c "$(curl -sS https://raw.githubusercontent.com/EasongChung/cdn/main/mht-arm64-install.sh)"
